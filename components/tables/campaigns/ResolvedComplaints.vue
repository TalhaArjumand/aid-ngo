<template>
  <div :class="{ 'pb-3': loading }">
    <div>
      <Modal id="beneficiary-complaint-resolved" title="Beneficiary complaint">
      ...
      <template v-else-if="complaints.length">
        ...
        <pagination
          :currentPageNum="resolvedComplaintPageNum"
          :totalNumOfItems="resolvedComplaintTotalItems"
...
export default {
  props: {
    ...
    resolvedComplaintPageNum: {
      type: Number,
      default: 1,
    },
    resolvedComplaintTotalItems: {
      type: Number,
      default: 0,
    },
  },
  ...
  methods: {
    viewComplaint(complaint) {
      ...
      this.$bvModal.show("beneficiary-complaint-resolved");
    },

    updatePage(action) {
      this.$emit("updatePage", { action, activeTab: "resolved" });
    },
  },
};
</script>
